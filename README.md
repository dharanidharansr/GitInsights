...
This is a **Next.js App Router** project and can be deployed to Vercel directly without Docker.

1. Import the repository in Vercel
2. Framework preset: **Next.js** (auto-detected)
3. Deploy

### Docker (For local production parity or non-Vercel hosts)

Build and run locally:

```bash
docker build -t gitinsights .
docker run --rm -p 3000:3000 --env-file .env gitinsights
```

Then open `http://localhost:3000`.

> ℹ️ Vercel does not deploy arbitrary Docker containers as the primary deployment model for Next.js apps. Use source-based deployment on Vercel; use Docker for local testing or other container platforms.
